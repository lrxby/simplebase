...
class Point2RBoxV2Head(AnchorFreeHead):
    ...
    def loss_by_feat(
        ...
        # Initialize NAOA loss
        loss_naoa_value = pos_bbox_preds.sum() * 0.0
        ...
        if len(pos_inds) > 0:
            ...
            # decay_factor = max(0.0, 1.0 - (self.epoch - self.loss_naoa.warmup_epoch) / (12 - self.loss_naoa.warmup_epoch))
            # [Step C] Calculate NAOA Loss
            # We use pos_rbox_preds which contains (cx, cy, w, h, theta)
            ...
            loss_naoa_value = pos_bbox_preds.sum()
