...
# [修改 1] 增大 Batch Size 并优化数据加载
train_dataloader = dict(
    batch_size=8,             # 原为 2，提升到 8 以跑满 A5000 显存
    num_workers=8,            # 增加数据加载线程，防止 CPU 瓶颈
    persistent_workers=True,  # 保持 worker 进程存活，减少重建开销
    dataset=dict(pipeline=train_pipeline)
)

# [修改 2] 开启混合精度训练 (AMP) 并线性缩放学习率
optim_wrapper = dict(
    # type='AmpOptimWrapper',   # 开启 FP16 混合精度，加速计算并省显存
    # loss_scale='dynamic',     # 动态 Loss Scale 防止梯度溢出
    optimizer=dict(
        _delete_=True,
        type='AdamW',
        lr=0.0002,            # 线性缩放：0.00005 * (8/2) = 0.0002
        betas=(0.9, 0.999),
        weight_decay=0.05),
