...
# ================= 配置区域 =================
# 1. DroneVehicle 数据集路径（三个子集独立拆分，不合并）
DRONE_ROOT = '/mnt/data/xiekaikai/DroneVehicle'
TRAIN_DIR = os.path.join(DRONE_ROOT, 'train', 'annfiles')    # 训练集独立路径
VAL_DIR = os.path.join(DRONE_ROOT, 'val', 'annfiles')      # 验证集独立路径
TEST_DIR = os.path.join(DRONE_ROOT, 'test', 'annfiles')    # 测试集独立路径

# 2. 对比模式（保持不变）
MODES = ['log', 'linear', 'sqrt', 'square']

# 3. 数据集类别（DroneVehicle官方类别）
CLASSES = ('car', 'bus', 'truck', 'van', 'freight_car')
RIDGE_LAMBDA = 1e-4  # 与 Loss 保持一致
EPS = 1e-6           # 与 Loss 的 clamp 保持一致

# 4. 结果保存路径
SAVE_DIR = '/mnt/data/liurunxiang/workplace/point2rbox-v2-UAV/loss2/val/DroneVehicle'
...
def parse_txt(txt_path):
    """解析 GT 数据（适配DroneVehicle标注格式：8坐标+类别ID）"""
    bboxes = []
    labels = []
...
# ================= 新增：统计结果合并函数（修复语法错误） =================
def merge_two_stats(stats_a, stats_b):
    """合并两个数据集的统计结果，拼接所有指标列表"""
    metrics = ['abs', 'rel', 'mse', 'iou']
    merged = {m: {c: {met: [] for met in metrics} for c in CLASSES} for m in MODES}
    for mode in MODES:
        for cls in CLASSES:
            for met in metrics:
                # 拼接两个数据集的指标数据
                merged[mode][cls][met] = stats_a[mode][cls][met] + stats_b[mode][cls][met]
    return merged

def merge_multiple_stats(stats_list):
    """合并多个数据集的统计结果（支持train+val+test）"""
    if not stats_list:
        metrics = ['abs', 'rel', 'mse', 'iou']
        return {m: {c: {met: [] for met in metrics} for c in CLASSES} for m in MODES}
    # 依次合并所有统计结果
    ...
    for stats in stats_list[1:]:
        final_stats = merge_two_stats(final_stats, stats)
    return final_stats
...
def main():
    ...
    # 第一步：独立统计三个数据集
    stats_train = collect_dataset_stats(TRAIN_DIR, "Train Set")
    stats_val = collect_dataset_stats(VAL_DIR, "Val Set")
    stats_test = collect_dataset_stats(TEST_DIR, "Test Set")
    
    # 第二步：生成三份独立子集报告
    print_and_save_stats(
        stats_train,
        title="详细对比报告: Train Set | 指标：IoU(↑), MAE(px)(↓), MAPE(%)(↓), MSE(↓)",
        save_filename="train_set_report.txt"
    )
    print_and_save_stats(
        stats_val,
        title="详细对比报告: Val Set | 指标：IoU(↑), MAE(px)(↓), MAPE(%)(↓), MSE(↓)",
        save_filename="val_set_report.txt"
    )
    print_and_save_stats(
        stats_test,
    ...
    # 第三步：合并三个数据集，生成全局汇总统计报告
    stats_global = merge_multiple_stats([stats_train, stats_val, stats_test])
    print_and_save_stats(
        stats_global,
        title="详细对比报告: Global(Train+Val+Test) | 指标：IoU(↑), MAE(px)(↓), MAPE(%)(↓), MSE(↓)",
