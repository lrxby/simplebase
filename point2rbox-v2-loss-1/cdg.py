...
OUTPUT_DIR = 'work_dirs/loss_statistics_report_codrone_gating' # 结果保存在新目录
...
def calculate_naoa_loss_vectorized(objects):
    ...
    # 6. [关键修改] 应用长宽比门控
    # 对 CODrone 尤为重要，可自动忽略 people, traffic-light 等非方向性物体的 Loss
...
def plot_histogram_advanced(loss_data, class_name, save_dir, stats):
    ...
    plt.title(f'CODrone GT Loss (Gating > {AR_THRESHOLD}): {class_name}', fontsize=20)
    ...
    save_path = os.path.join(save_dir, f'{class_name}_dist_codrone_gating.png')
...
def main():
    ...
    print(f"找到 {len(label_files)} 个 CODrone 标注文件。开始全量统计 (Gating 模式)...")
    ...
    report_path = os.path.join(OUTPUT_DIR, 'statistics_report_codrone_gating.txt')
    ...
    print("="*160)
    print(f"CODrone (Gating) 报告已生成: {report_path}")
