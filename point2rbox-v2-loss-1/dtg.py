...
OUTPUT_DIR = 'work_dirs/loss_statistics_report_dota_gating'
...
# [New] 长宽比门控阈值
# 只有长宽比 > 1.3 的物体才计算方向 Loss
# 这将完美解决 Plane (ratio~1.1) Loss 虚高的问题，同时保留对 Vehicle (ratio>2) 的约束
AR_THRESHOLD = 1.3
...
def parse_dota_poly(poly, class_name):
    ...
    # [Strict] 模拟 Head 层对 square_cls 的强制归零
    ...
    # [New] 计算长宽比 (Aspect Ratio)
    # 始终取 长边 / 短边，保证 ratio >= 1.0
    ratio = max(w, h) / (min(w, h) + 1e-6)

    return {'cx': cx, 'cy': cy, 'theta': theta, 'scale': scale, 'ratio': ratio}

def calculate_naoa_loss_vectorized(objects):
    """
    [Modified] 计算一张图中所有目标的 Loss 列表
    修改点：引入长宽比门控 (Aspect Ratio Gating)
    """
    N = len(objects)
    if N < 2:
        return np.array([]) 
    ...
    ratios = np.array([obj['ratio'] for obj in objects]) # [New] 获取长宽比

    # 2. 角度矢量化 (保持 2*theta 原始定义，不搞 4*theta)
    ...
    raw_loss_per_item = consistency_weights * (1.0 - cos_sim)
    
    # 6. [关键修改] 应用长宽比门控
    # 如果 ratio > 1.3，gate = 1 (计算 Loss)
    # 如果 ratio <= 1.3 (飞机/油罐)，gate = 0 (Loss 归零)
    ar_gate = (ratios > AR_THRESHOLD).astype(np.float32)
    
    final_loss = raw_loss_per_item * ar_gate
...
def plot_histogram_advanced(loss_data, class_name, save_dir, stats):
    ...
    plt.title(f'GT Loss Distribution (AR Gating > {AR_THRESHOLD}): {class_name}', fontsize=20)
    ...
    save_path = os.path.join(save_dir, f'{class_name}_dist_v7.png')
...
def main():
    ...
    print(f"找到 {len(label_files)} 个标注文件。开始全量统计 (Gating 模式)...")
    ...
    report_path = os.path.join(OUTPUT_DIR, 'statistics_report_gating.txt')
    ...
    print("="*160)
    print(f"Gating模式报告已生成: {report_path}")
