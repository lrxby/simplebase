...
class PerspectiveAwareSizeConsistencyLoss(nn.Module):
    """Perspective-Aware Size Consistency Loss (PASCL).

    It learns the perspective mapping between position (y) and size (sqrt(area))
    for rigid objects. For orthographic datasets (e.g. DOTA), it automatically
    degenerates to size consistency loss.

    Args:
        loss_weight (float): Weight of loss. Defaults to 1.0.
        beta (float): Beta parameter for SmoothL1Loss. Defaults to 1.0.
        eps (float): Small value to prevent division by zero. Defaults to 1e-6.
        target_classes (list[int] | None): List of class indices to apply the loss. 
            If None, the loss is not applied (or applied to all if logic dictates).
            Defaults to None.
    """

    def __init__(self, loss_weight=1.0, beta=1.0, eps=1e-6, target_classes=None):
    ...
    def forward(self, pred_bboxes, scores):
        """Forward function.

        Args:
            pred_bboxes (Tensor): Shape (N, 5), (x, y, w, h, theta)
            scores (Tensor): Shape (N, ), confidence scores

        Returns:
            torch.Tensor: The calculated loss
        """
        # Less than 2 objects cannot fit a line
        if len(pred_bboxes) < 2:
            return pred_bboxes.new_tensor(0.0)

        # 1. Prepare data
        # y: center y, s: sqrt(area)
        y = pred_bboxes[:, 1]
        w = pred_bboxes[:, 2].clamp(min=1e-6)
        h = pred_bboxes[:, 3].clamp(min=1e-6)
        s = torch.sqrt(w * h)
        u = scores
        ...
        y_bar = torch.sum(u * y) / sum_u
        s_bar = torch.sum(u * s) / sum_u

        # 3. Fit slope k (Weighted Least Squares)
        y_diff = y - y_bar
        numerator = torch.sum(u * y_diff * (s - s_bar))
        denominator = torch.sum(u * (y_diff ** 2)) + self.eps
        
        # 4. Adaptive logic: if denominator is small (vertical distribution) or no perspective, k->0
        k = numerator / denominator

        # 5. Fit intercept b
        b = s_bar - k * y_bar

        # 6. [Critical] Gradient Detachment: Stop gradient from flowing back to k and b
        k = k.detach()
        b = b.detach()

        # 7. Calculate expected size
        s_hat = k * y + b

        # 8. Smooth L1 Loss and weighted average
        loss = self.smooth_l1(s, s_hat)
        weighted_loss = torch.sum(u * loss) / sum_u
