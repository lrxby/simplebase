...
#     Args:
#         p (float): Noise ratio relative to image size. Default: 0.1.
#     """
...
#         # 基于图像尺寸计算噪声基准
#         img_shape = results['img_shape']  # (height, width) or (height, width, channel)
#         img_size = torch.tensor([img_shape[1], img_shape[0]], dtype=torch.float32)  # [width, height]
#         noise_base = torch.min(img_size)  # 使用图像的最小边长作为基准
            
#         # 添加噪声
#         noise_x = (torch.rand(1) * 2 - 1) * self.p * noise_base
#         noise_y = (torch.rand(1) * 2 - 1) * self.p * noise_base
            
#         results['gt_bboxes'].tensor[:, 0] += noise_x
#         results['gt_bboxes'].tensor[:, 1] += noise_y
