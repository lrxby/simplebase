...
# --- 数据集配置 ---
CLASSES = ('car', 'truck', 'bus', 'traffic-light', 'traffic-sign', 'bridge', 'people', 'bicycle', 'motor', 'tricycle', 'boat', 'ship')
DATASET_PATH = "/mnt/data/xiekaikai/split_ss_codrone"
SAVE_DIR = "/mnt/data/liurunxiang/workplace/point2rbox-v2-loss-new/global_report_cd_area"
...
def run_full_analysis(base_path):
    ...
    print("-" * 80)
    for cat in CLASSES:
        if not cat_global_areas[cat]: continue
        g_cv = np.std(cat_global_areas[cat]) / (np.mean(cat_global_areas[cat]) + 1e-6)
...
def plot_grid_summary(data_dict, title, xlabel, filename):
    ...
    for i, cat in enumerate(valid_cats):
        axes[i].hist(data_dict[cat], bins=40, color='skyblue', edgecolor='black', alpha=0.7)
