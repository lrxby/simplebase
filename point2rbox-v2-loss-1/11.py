...
# # ================= 配置区域（适配DroneVehicle）=================
# # 数据集标注路径
# ANNOTATION_PATH = '/mnt/data/xiekaikai/DroneVehicle/train/annfiles'
# # 结果保存文件夹
# OUT_DIR = 'work_dirs/analysis_results_dronevehicle5'
# # DroneVehicle类别映射（数字→名称）
# CLASS_MAP = {
#     0: 'car',
#     1: 'bus',
#     2: 'truck',
...
#     4: 'feright_car'
# }
# RIGID_CLASSES = list(CLASS_MAP.values())  # 验证所有类别
# # 统计阈值（和DOTA保持一致，便于对比）
...
# def parse_dronevehicle_line(line):
#     """解析DroneVehicle标注行（适配数字类别+无难度字段）"""
#     parts = line.strip().split()
#     # DroneVehicle标注格式：8坐标 + 数字类别（共9个字段）
#     if len(parts) != 9:
#         return None, None
...
#         # 提取数字类别并映射为名称
#         cls_num = int(parts[8])
#         if cls_num not in CLASS_MAP:
#             return None, None
#         category = CLASS_MAP[cls_num]
...
#     """生成文本报告：移除Sig Corr，新增总计行+平均绝对值Corr（适配DroneVehicle）"""
#     lines = []
...
#     """生成可视化图表：移除Sig Corr相关标注，新增Avg |Corr|可视化（适配DroneVehicle）"""
#     # 确保字符正常显示（适配英文/特殊符号）
#     plt.rcParams['font.sans-serif'] = ['DejaVu Sans']
#     plt.rcParams['axes.unicode_minus'] = False
    
#     # 图1: 类别CV箱线图（适配DroneVehicle标题）
...
#         plt.title('Scale Consistency (CV) Distribution per Class (DroneVehicle)', fontsize=12)
...
#     # 图2-1 分类别相关系数子图（适配DroneVehicle，新增Avg |Corr|标注）
#     valid_classes = [c for c in RIGID_CLASSES if len(stats[c]['cvs']) > 0]
#     n_classes = len(valid_classes)
#     if n_classes > 0:
#         # 适配5个类别：2行3列布局（空出最后一个子图）
#         n_rows = 2
#         n_cols = 3
...
#         plt.suptitle('Correlation (Y-Coord vs Scale) Distribution per Class (DroneVehicle)', fontsize=14)
#         plt.tight_layout(rect=[0, 0, 1, 0.96])
#         plt.savefig(os.path.join(out_dir, '2_corr_dist_per_class.png'), dpi=300)
#         plt.close()

#     # 图2-2 全类别统一相关系数直方图（适配DroneVehicle，新增Avg |Corr|标注）
...
#         plt.title(f'Correlation (Y-Coord vs Scale) Distribution (All Classes, DroneVehicle) (Total n={total_samples})', fontsize=12)
...
#         for idx, sample in enumerate(vis_samples[:12]):  # 最多展示12个样本
#             plt.subplot(n_rows, n_cols, idx+1)
...
#         plt.title('Average |Pearson Correlation| per Class (DroneVehicle)', fontsize=12)
#         plt.xlabel('Class', fontsize=10)
...
#     """生成Markdown报告：移除Sig Corr，新增总计行+平均绝对值Corr（适配DroneVehicle）"""
#     # 先计算总计数据
#     total_imgs = 0
#     all_cvs = []
#     all_corrs = []
...
#         for line in lines:
#             cat, data = parse_dronevehicle_line(line)
#             if cat and data:
...
ANNOTATION_PATH = '/mnt/data/xiekaikai/DroneVehicle/train/annfiles'
# 结果保存文件夹
OUT_DIR = 'work_dirs/analysis_results_dronevehicle2'
# DroneVehicle类别映射（数字→名称）
CLASS_MAP = {
    0: 'car',
    1: 'bus',
...
def generate_text_report(stats, out_path):
    """生成文本报告：保留原有统计，新增正负相关数量+正负相关均值（适配DroneVehicle）"""
    lines = []
    lines.append("="*180)  # 扩展分隔线长度适配新列
    ...
    lines.append("2. Avg Corr > 0.6 或 Avg |Corr| > 0.6 -> 强透视性（近大远小）(适合用线性回归约束)")
    lines.append("3. PosRatio(%) > 70% -> 多数样本符合近大远小规律")
    lines.append("4. 都不满足                            -> 建议用局部邻域一致性约束")
...
def generate_markdown_report(stats, out_dir):
    """生成Markdown报告：保留原有统计，新增正负相关数量+正负相关均值（适配DroneVehicle）"""
    # 先计算总计数据
    total_imgs = 0
    all_cvs = []
    all_corrs = []
