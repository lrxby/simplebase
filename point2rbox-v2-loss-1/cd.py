...
OUTPUT_DIR = 'work_dirs/loss_statistics_report_codrone'
...
def parse_poly_to_rbox(poly, class_name):
    ...
    scale = np.sqrt(w * h)
    # CODrone 目标可能非常小(如 people)，clip 下限设为 1.0 以防万一
...
def plot_histogram_advanced(loss_data, class_name, save_dir, stats):
    ...
    plt.title(f'CODrone GT Loss Distribution: {class_name}', fontsize=20)
    ...
    save_path = os.path.join(save_dir, f'{class_name}_dist_codrone.png')
...
def main():
    ...
    print(f"找到 {len(label_files)} 个 CODrone 标注文件。开始全量统计...")
    ...
    for label_file in tqdm(label_files):
        ...
        for line in lines:
            ...
            # 至少需要 10 个元素 (8 coords + 1 class + 1 diff)
    ...
    report_path = os.path.join(OUTPUT_DIR, 'statistics_report_codrone.txt')
    ...
    print("="*160)
    print(f"CODrone 统计报告已生成: {report_path}")
