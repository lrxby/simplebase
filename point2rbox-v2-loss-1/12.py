...
# ================= 配置区域（适配CODrone）=================
# 数据集标注路径
ANNOTATION_PATH = '/mnt/data/xiekaikai/split_ss_codrone/trainval/annfiles'
# 结果保存文件夹
OUT_DIR = 'work_dirs/analysis_results_codrone2'
# CODrone目标类别（过滤ignored等无效类别）
RIGID_CLASSES = (
    'car', 'truck', 'bus', 'traffic-light',
    'traffic-sign', 'bridge', 'people', 'bicycle',
    'motor', 'tricycle','boat', 'ship'
)
# 统计阈值（和DOTA/DroneVehicle保持一致，便于对比）
...
def parse_codrone_line(line):
    """解析CODrone标注行（适配8坐标+类别名+难度值格式，优化异常处理）"""
    parts = line.strip().split()
    # CODrone标注格式：8坐标 + 类别名称 + 难度值（共10个字段）
    if len(parts) < 10:
        return None, None
    
    try:
        # 提取类别名称，过滤无效类别（如ignored）
        ...
        # 计算中心Y坐标
        cy = np.mean(poly[:, 1])
        # 鞋带公式计算面积 -> 开根号得到scale（和DOTA/DroneVehicle保持一致）
        x, y = poly[:, 0], poly[:, 1]
...
def calculate_stats_per_image(img_data):
    ...
    for cls_name, points in img_data.items():
        ...
        # 1. 计算尺寸一致性：变异系数CV（标准差/均值）
        mean_s = np.mean(scales)
        std_s = np.std(scales)
        cv = std_s / (mean_s + 1e-6)  # 避免除零
...
def generate_text_report(stats, out_path):
    """生成文本报告：保留原有统计，新增Pos_Count/Neg_Count/Avg_Corr_Pos/Avg_Corr_Neg（适配CODrone）"""
    lines = []
    lines.append("="*180)  # 扩展分隔线长度适配新增列
...
def generate_visualizations(stats, vis_samples, out_dir):
    """生成可视化图表：移除Sig Corr相关标注，新增Avg |Corr|可视化（适配CODrone）"""
    # 确保字符正常显示
    ...
    if cv_data:
        ...
        plt.title('Scale Consistency (CV) Distribution per Class (CODrone) - All Samples', fontsize=12)
    ...
    if n_classes > 0:
        ...
        plt.suptitle('Correlation (Y-Coord vs Scale) Distribution per Class (CODrone) - All Samples', fontsize=14)
    ...
    if all_corr_data:
        ...
        plt.title(f'Correlation (Y-Coord vs Scale) Distribution (All Classes, CODrone) (Total n={total_samples})', fontsize=12)
    ...
    if cls_abs_corr:
        ...
        plt.title('Average |Pearson Correlation| per Class (CODrone) - All Samples', fontsize=12)
        plt.xlabel('Class', fontsize=10)
...
def generate_markdown_report(stats, out_dir):
    """生成Markdown报告：保留原有统计，新增Pos_Count/Neg_Count/Avg_Corr_Pos/Avg_Corr_Neg（适配CODrone）"""
    # 先计算总计数据
    total_imgs = 0
    all_cvs = []
    all_corrs = []
...
def verify_prior():
    ...
    for txt_file in tqdm(txt_files, desc='解析标注并统计'):
        ...
        for line in lines:
            cat, data = parse_codrone_line(line)
            if cat and data:
