...
# [新增] 固定随机种子配置
# seed: 随机种子数值
# deterministic: 是否强制使用确定性算法（True会变慢但更可复现，False速度快）
...
model = dict(
    ...
    bbox_head=dict(
        ...
        # [New Config] E-NAOA Loss
        loss_naoa=dict(
            type='NAOALoss',
            loss_weight=0.2,      # Loss Weight
            warmup_epoch=0,       # Start epoch (0-based)
            alpha=1.0,            # Score weighting power
            k_radius=2.0          # Neighborhood radius factor
        )),
...
train_pipeline = [
    ...
    # dict(type='RBox2PointWithNoise', p=1.0),
