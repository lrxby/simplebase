...
def generate_report(all_results):
    # 1. 基础分析报告 (基于 S_area)
    print("\n" + "="*110)
    print(f"{'DATASET':<12} | {'CLASS':<16} | {'COUNT':<8} | {'CV(Avg)':<9} | {'CV(Med)':<9} | {'Corr(Area)':<10} | {'PosRatio':<8}")
    print("="*110)
    ...
    for d_name, (d_stats, _) in all_results.items():
        ...
        for cls_name in sorted_classes:
            ...
            med_cv = np.median(metrics['cv']) if n > 0 else 0
            valid_corrs = [c for c in metrics['corr'] if not np.isnan(c)]
            count = d_stats[cls_name]['count']
            ...
            print(f"{d_name:<12} | {cls_name:<16} | {count:<8} | {m_cv:.3f}     | {med_cv:.3f}     | {m_corr:.3f}      | {pos_ratio:.2f}")
        ...
        if total_cnt > 0:
            overall_cv_avg = total_cv / total_cnt
            overall_cv_med = np.median(all_cv) if all_cv else 0
            print("-" * 110)
            print(f"{d_name:<12} | {'[OVERALL]':<16} | {total_cnt:<8} | {overall_cv_avg:.3f}     | {overall_cv_med:.3f}     | {total_corr/total_cnt:.3f}      | -")
            print("=" * 110)
