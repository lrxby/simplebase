...
OUTPUT_DIR = 'work_dirs/vis_dronevehicle'
...
TARGET_CLASSES = list(ID_TO_CLASS.values())
TOP_K = 10

# [Strict] Square Classes (无方向物体)
# DroneVehicle 全是车，通常都有方向，设为空
SQUARE_CLASSES = []
...
def main():
    ...
    print(f"找到 {len(label_files)} 个标注文件。开始 DroneVehicle 物理验证...")
    ...
    for cls in TARGET_CLASSES:
        ...
        for i in range(min(TOP_K, len(data))):
            item = data[i]
            img_path = os.path.join(IMG_DIR, item['img_id'] + '.jpg') # DroneVehicle 主要是jpg, draw_results会自动检查
        ...
        for i in range(min(TOP_K, len(data))):
            item = data[-(i+1)] 
            img_path = os.path.join(IMG_DIR, item['img_id'] + '.jpg')
