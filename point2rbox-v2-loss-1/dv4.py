...
OUTPUT_DIR = 'work_dirs/loss_statistics_report_dronevehicle_4theta' # 结果保存在新目录
...
# 4-Theta 模式下，所有物体都具备 90度对称性，此处仅用于强制归零逻辑(若需要)
SQUARE_CLASSES = [] 
# ==============================================================
...
def plot_histogram_advanced(loss_data, class_name, save_dir, stats):
    ...
    plt.title(f'DroneVehicle GT Loss (4-Theta Mode): {class_name}', fontsize=20)
    ...
    save_path = os.path.join(save_dir, f'{class_name}_dist_dv_4theta.png')
...
def main():
    ...
    print(f"找到 {len(label_files)} 个 DroneVehicle 标注文件。开始全量统计 (4*theta 模式)...")
    ...
    for label_file in tqdm(label_files):
        ...
        for line in lines:
            ...
            try:
                ...
                class_id = int(parts[8]) # DroneVehicle 是数字ID
    ...
    report_path = os.path.join(OUTPUT_DIR, 'statistics_report_dv_4theta.txt')
    ...
    print("="*160)
    print(f"DroneVehicle (4-Theta) 报告已生成: {report_path}")
