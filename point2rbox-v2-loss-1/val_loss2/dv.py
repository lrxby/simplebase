...
OUTPUT_DIR = 'dronevehicle'

CLASSES = ('car', 'bus', 'truck', 'van', 'freight_car')

# DroneVehicle 中所有类别均为刚性物体，全部加入
...
def process_single_file(file_path):
    """
    处理单个 DroneVehicle 标注文件
    格式: x1 y1 x2 y2 x3 y3 x4 y4 class_id
    """
    ...
    try:
        ...
        for line in lines:
            ...
            try:
                ...
                if class_id < 0 or class_id >= len(CLASSES): continue
...
def plot_histogram_advanced(loss_data, class_name, save_dir, stats):
    ...
    n, bins, patches = plt.hist(display_data, bins=bins, color='#66bb6a', edgecolor='black', alpha=0.8)
...
def main():
    ...
    files = []
    print("Scanning DroneVehicle files...")
