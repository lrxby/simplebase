...
OUTPUT_DIR = 'work_dirs/loss_statistics_report_dronevehicle_gating' # 结果保存在新目录
...
# DroneVehicle 类别定义 (索引 0-4)
TARGET_CLASSES = ['car', 'bus', 'truck', 'van', 'feright_car']

# [Strict] Square Classes
# Gating 模式下，主要依赖 AR_THRESHOLD 自动判断，此处保留空列表或特定强制类别
SQUARE_CLASSES = [] 
# ==============================================================
...
def calculate_naoa_loss_vectorized(objects):
    ...
    ratios = np.array([obj['ratio'] for obj in objects]) # [New]

    # 2. 角度矢量化 (保持 2*theta, 依赖 Gating 消除歧义)
...
def plot_histogram_advanced(loss_data, class_name, save_dir, stats):
    ...
    plt.title(f'DroneVehicle GT Loss (Gating > {AR_THRESHOLD}): {class_name}', fontsize=20)
    ...
    save_path = os.path.join(save_dir, f'{class_name}_dist_dv_gating.png')
...
def main():
    ...
    print(f"找到 {len(label_files)} 个 DroneVehicle 标注文件。开始全量统计 (Gating 模式)...")
    ...
    report_path = os.path.join(OUTPUT_DIR, 'statistics_report_dv_gating.txt')
    ...
    print("="*160)
    print(f"DroneVehicle (Gating) 报告已生成: {report_path}")
