...
# # ==============================================================================
# # 1. 配置区域 (Row 5 PCA2 Best Params)
# # ==============================================================================
# DATA_ROOT = '/mnt/data/xiekaikai/split_ss_dota/trainval/'
# SAVE_PATH = '/mnt/data/liurunxiang/workplace/point2rbox-v2-baseline/dataset/dota/trainval.pkl'
# DEVICE = 'cuda:1'

# # Row 5 最佳参数
# CFG = dict(sigma=2048, ds=2, pos=0.99, neg=0.1)
...
#     """
#     Row 5 专用逻辑: PCA 次轴 (Second Principal Component)
#     """
#     J = len(gt_points)
#     sigmas_list = []
#     angles_list = []
...
#         # 排序：从小到大。Row 5 使用次轴 (eigenvalue 小的那个)
#         # 注意：Row 4 用 v_main (idx 1)，Row 5 用 v_sub (idx 0)
#         # eigh 返回的是升序
#         v_sub = eig_vecs[:, 0] # 次轴
#         theta_pca2 = np.arctan2(v_sub[1], v_sub[0])
        
#         # 构建 Stage 2 需要的 Sigma (利用投影测量)
...
#         # 注意：这里我们构建 Sigma 是为了 Stage 2 注水
#         # 我们希望生成的 Mask 沿着 theta_pca2 方向生长 (或收缩)
#         # 所以传入 rbox2sigma 的 W 对应 theta 方向
...
#     print(f"Start Generating Pseudo Labels for DOTA-v1.0 (Row 5 - PCA Minor Axis)...")
#     print(f"Config: {CFG}")
...
#         # 3. Geometry Analysis (Row 5 - PCA Minor Axis)
...
#             center = m_pts.mean(0) # 使用 Mask 重心来测量可能更准，但在保存时还是用 GT 中心 (点监督逻辑)
#             angle = angles_s2[idx]
            
#             # 投影测量
#             w_raw, h_raw = get_watershed_wh_original_loss(m_pts, center, angle)
            
#             # --- 核心：Post-Process (直接应用) ---
#             if label_id in POST_PROCESS:
#                 scale = POST_PROCESS[label_id]
#                 w_raw *= scale
#                 h_raw *= scale
                
#             # --- 核心：Long Edge Definition (LED) 强制对齐 ---
#             # 确保 W >= H，这是为了配合后续训练中常用的 IoU Loss 定义
#             if h_raw > w_raw:
#                 w_raw, h_raw = h_raw, w_raw
#                 angle += (np.pi / 2) # 弧度制加 90度
            
#             # 归一化角度到 [-pi, pi) 或 MMRotate 标准
#             # 这里简单做个周期处理
...
# # 1. 配置区域 
# # ==============================================================================
# DATA_ROOT = '/mnt/data/liurunxiang/workplace/point2rbox-v2-baseline/dataset/dota/trainval/'
# SAVE_PATH = '/mnt/data/liurunxiang/workplace/point2rbox-v2-baseline/dataset/dota/dota1-PCA.pkl'
# DEVICE = 'cuda:5'
