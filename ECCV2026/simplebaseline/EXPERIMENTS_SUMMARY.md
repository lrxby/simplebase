...
## 三.5、论文对齐单变量实验（align 系列）结果 —— 2026-09-12 完成

**实验设计**（用户拍板）：相对最优配置 **ow3（OurWaterLoss 权重=3，seed0=0.4834）**，对"论文明确写了、代码明确不一样"的 7 处差异做**单变量**修改，每处只跑一次 seed0，验证"按论文实现是否掉点"。采纳阈值 **best ≥ 0.478**（相对 ow3 降 ≤0.5 点）。

**实现方式**：给 AngleLoss / SizeLoss / OurWaterLoss 增加"论文对齐开关"参数，**默认值 = 当前工程行为**（保证已跑 ow3/ow5 权重语义不变）；6 个 `align-*.py` 配置各自只开启一处论文版。原版备份 `/tmp/point2rbox_v2_loss.py.bak_align`、`/tmp/audit-fixed-ow3.py.bak_align`。

| 差异项 | 论文版 | 工程版（默认） | 实验 | best mAP@ep | 相对 ow3 | 结论 |
|---|---|---|---|---|---|---|
| ⑤ ε | ε=0.01 | 1e-6 | align-eps | **0.4865**@12 | +0.31 | ✅ **采纳论文版** |
| ④ SizeLoss 形式 | L2 (MSE) | SmoothL1(β=1.0) | align-sizel2 | **0.4808**@10 | −0.26 | ✅ **采纳论文版** |
| ② AngleLoss 邻域 | 曼哈顿 /(2wh) | 欧氏平方 exp(−d²/8wh) | align-neighbor | 0.4683@11 | −1.51 | ❌ 保留工程版 |
| ③ SizeLoss 空间 | 线性直接回归 w | log 空间 | align-sizelinear | 0.2914@11 | −19.20 | ❌ 保留工程版（log 空间碾压线性） |
| ⑥ Slack | 丢 top-10% | topk=0.95（丢 5%） | align-slack | **0.4975**@12 | +1.41 | ✅ **采纳论文版**（反超 ow3） |
| ① 权重 | 0.2/0.1/0.5（隐藏缩放） | 1.0/1.0/3.0 | align-weights | 0.4772@12 | −0.62（差 0.0008 未达 0.478） | ❌ 保留工程版权重 |

**最终公开配置决定**：ε=0.01、SizeLoss 用 L2、OurWaterLoss 用 top-10% slack（3 处按论文）；权重、AngleLoss 邻域公式、SizeLoss 回归空间（log）保留工程版（3 处有实验依据保留）。全部 6 个实验 12ep 完整跑完，无 NaN/OOM；训练期间 NVIDIA 驱动 segfault 频发，auto_heal 守护自动 resume 兜底（共 7 次自动恢复），结果均以最终 12ep 为准。

> 备注：align-weights 与阈值仅差 0.08 个百分点，属边界情况；是否采纳论文权重可留给 codex 最终裁定，当前默认保留工程版。
...
- **两卡 GPU 空闲**：align 系列 6 个单变量实验（eps/sizel2/neighbor/sizelinear/slack/weights）已于 2026-09-12 04:29 全部 12ep 跑完，无异常。
- 调度器保持停止（15:13:03），`decided_g3.flag` 已生成——不自动启动新实验，等 codex 确认后续方案。
- align 补丁与 6 个 align-*.py 配置已提交至 `fix/pseudo-geometry-eval` 分支（commit 见 git log），GitHub 已同步。
