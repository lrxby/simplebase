...
class Point2RBoxV2Head(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_pascl: Optional[ConfigType] = None,
        ...
        if loss_pascl is not None:
            self.loss_pascl = MODELS.build(loss_pascl)
        else:
            self.loss_pascl = None
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            ins_cls_scores = pos_cls_scores.new_zeros(*bid.shape).index_reduce_(
            ...
            loss_pascl = ori_mu_all.new_tensor(0)
            ...
            for batch_id in range(len(batch_gt_instances)):
                ...
                # PASCL
                if self.loss_pascl is not None and getattr(self.loss_pascl, 'target_classes', None) is not None:
                     # Get instance data for this image
                     # We use ins_rbox_preds which are (x, y, w, h, theta)
                     rboxes = ins_rbox_preds[group_mask]
                     scores = ins_cls_scores[group_mask]
                     labels_per_img = ins_labels[group_mask]
                     
                     for cls_id in self.loss_pascl.target_classes:
                         cls_mask = labels_per_img == cls_id
                         if cls_mask.sum() >= 2:
                             loss_pascl += self.loss_pascl(rboxes[cls_mask], scores[cls_mask])
            ...
            loss_pascl = loss_pascl / len(batch_gt_instances)
