...
class DOTADataset(BaseDataset):
    ...
    Args:
        ...
        max_samples (int, optional): The maximum number of samples to load.
            If None, load all samples. Defaults to None.
    """
    ...
    def __init__(self,
                 diff_thr: int = 100,
                 img_suffix: str = 'png',
                 max_samples: int = None,  # [修改] 新增参数
                 **kwargs) -> None:
        self.diff_thr = diff_thr
        self.img_suffix = img_suffix
        self.max_samples = max_samples  # [修改] 保存参数
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if self.ann_file == '':
            ...
            # [修改] 限制 json 条目数量
            if isinstance(root, list) and self.max_samples is not None:
                root = root[:self.max_samples]
            ...
            # [修改] 限制 txt 文件数量
            if self.max_samples is not None:
                print(f'\n[DEBUG DOTADataset] Limiting dataset size to {self.max_samples} samples.')
                txt_files = txt_files[:self.max_samples]
