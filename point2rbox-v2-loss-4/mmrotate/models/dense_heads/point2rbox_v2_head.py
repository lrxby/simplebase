...
class Point2RBoxV2Head(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_cls: ConfigType = dict(
                     ...
                     alpha=0.25,
                     loss_weight=1.0),
                 loss_bbox: ConfigType = dict(type='GDLoss', loss_type='gwd', loss_weight=5.0),
                 loss_overlap: ConfigType = dict(type='GaussianOverlapLoss', loss_weight=10.0),
                 loss_voronoi: ConfigType = dict(type='VoronoiWatershedLoss', loss_weight=5.0),
                 loss_bbox_edg: ConfigType = dict(type='EdgeLoss', loss_weight=0.3),
                 loss_ss=dict(type='Point2RBoxV2ConsistencyLoss', loss_weight=1.0),
                 loss_size: ConfigType = dict(type='SizeLoss', loss_weight=1.0),     # [修改] 重命名
                 loss_angle: ConfigType = dict(type='AngleLoss', loss_weight=1.0),   # [修改] 重命名
                 loss_ourwater: ConfigType = dict(type='OurWaterLoss', loss_weight=4.0), # [新增]
                 
                 # ------------------ 独立的 Loss 拔插开关 ------------------
                 use_loss_bbox: bool = True,
                 use_loss_overlap: bool = True,
                 use_loss_voronoi: bool = True,
                 use_loss_bbox_edg: bool = True,
                 use_loss_ss: bool = True,
                 use_loss_size: bool = True,
                 use_loss_angle: bool = True,
                 use_loss_ourwater: bool = True,
        ...
        # [修改] 绑定布尔开关属性
        self.use_loss_bbox = use_loss_bbox
        self.use_loss_overlap = use_loss_overlap
        self.use_loss_voronoi = use_loss_voronoi
        self.use_loss_bbox_edg = use_loss_bbox_edg
        self.use_loss_ss = use_loss_ss
        self.use_loss_size = use_loss_size
        self.use_loss_angle = use_loss_angle
        self.use_loss_ourwater = use_loss_ourwater
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        """
        [修改] 重写父类 loss 方法，为了显式传递 batch_data_samples 给 loss_by_feat
        """
        outputs = self(x)
        ...
        return self.loss_by_feat(
            *outputs,
            ...
            batch_data_samples=batch_data_samples)  # [新增透传]
    ...
    def loss_by_feat(
        ...
        batch_data_samples: SampleList = None  # [修改] 接收样本用于提取伪标签
    ) -> Dict[str, Tensor]:
        """Calculate the loss based on the features extracted by the detection head."""
        
        # --- [新增] Step 0: Extract Pseudo Labels ---
        batch_pseudo_boxes = []
        ...
        flatten_pseudo_targets = torch.cat(pseudo_targets) # [新增]
        flatten_gt_inds = torch.cat(gt_inds)               # [新增]
        ...
        pos_pseudo_targets = flatten_pseudo_targets[pos_inds] # [新增]
        pos_gt_ids = flatten_gt_inds[pos_inds]                # [新增]

        self.vis = [None] * len(batch_gt_instances)  # For visual debug
        
        # [新增] 初始化条件隔离占位符，防御 DDP 报错
        dummy_zero_loss = pos_bbox_preds.sum() * 0.0 

        val_loss_bbox = dummy_zero_loss.clone()
        val_loss_ovl = dummy_zero_loss.clone()
        val_loss_vor = dummy_zero_loss.clone()
        val_loss_edg = dummy_zero_loss.clone()
        val_loss_ss = dummy_zero_loss.clone()
        val_loss_size = dummy_zero_loss.clone()
        val_loss_angle = dummy_zero_loss.clone()
        val_loss_ourwater = dummy_zero_loss.clone()
        ...
        if len(pos_inds) > 0:
            ...
            _, bidx, bcnt = torch.unique(
                ins_bid_with_view.long(), return_inverse=True, return_counts=True)
            bmsk = bcnt[bidx] == 2
            ...
            for batch_id in range(len(batch_gt_instances)):
                ...
                # [包裹] 重叠惩罚
                if self.use_loss_overlap and len(mu) >= 2:
                    val_loss_ovl += self.loss_overlap((mu, sigma.bmm(sigma)))
                    
                # [包裹] 分水岭边界
                if self.use_loss_voronoi and len(mu) >= 1:
                    ...
                    val_loss_vor += self.loss_voronoi((mu, sigma.bmm(sigma)),
            ...
            # [包裹] Edge 边界对齐
            if self.use_loss_bbox_edg and self.epoch >= self.edge_loss_start_epoch:
                batched_rbox = []
                for batch_id in range(len(batch_gt_instances)):
                    group_mask = (ins_batch == batch_id) & (ins_bids != 0)
                ...
                val_loss_edg = self.loss_bbox_edg(batched_rbox, self.edges)
            
            # 平均化
            if self.use_loss_overlap: val_loss_ovl = val_loss_ovl / len(batch_gt_instances)
            if self.use_loss_voronoi: val_loss_vor = val_loss_vor / len(batch_gt_instances)
            if self.use_loss_bbox_edg: val_loss_edg = val_loss_edg / len(batch_gt_instances)

            # [包裹] 自监督 SS
            if self.use_loss_ss:
                pair_gaus_preds = ins_gaus_preds[bmsk].view(-1, 2, 2, 2)
                pair_labels = ins_labels[bmsk].view(-1, 2)[:, 0]
                ss_square_mask = torch.zeros_like(pair_labels, dtype=torch.bool)
                for c in self.square_cls:
                    ss_square_mask = torch.logical_or(ss_square_mask, pair_labels == c)
                ...
                if torch.any(valid):
                    ...
                    ss_square_mask = ss_square_mask[valid]
                    ori_angle = pair_angle_preds[valid, 0]
                    trs_angle = pair_angle_preds[valid, 1]
                    val_loss_ss = self.loss_ss(
                        (ori_gaus, ori_angle),
                        (trs_gaus, trs_angle),
                        ss_square_mask,
                        *ss_info)
                else:
                    val_loss_ss = 0 * pos_angle_preds.sum()
            
            # --- [终极修改] 仅对"真实原始物体"执行自定义物理 Loss ---
            # pos_bid_targets[:, 1] 代表 syn(synthetic)。
            # syn == 1 是被 Copy-Paste 随机贴上来的假物体，毫无上下文逻辑，必须被过滤掉！
            pos_batch_idxs = pos_bid_targets[:, 0]
            pos_ori_mask = pos_bid_targets[:, 1] == 0 

            if self.use_loss_size:
                val_loss_size = self.loss_size(
                    pred_bboxes=pos_rbox_preds[pos_ori_mask],
                    scores=pos_cls_scores[pos_ori_mask],
                    labels=pos_labels[pos_ori_mask],
                    batch_idxs=pos_batch_idxs[pos_ori_mask],
                    img_metas=batch_img_metas
                )

            if self.use_loss_angle:
                val_loss_angle = self.loss_angle(
                    pos_bbox_preds=pos_rbox_preds[pos_ori_mask],
                    pos_scores=pos_cls_scores[pos_ori_mask],
                    pos_labels=pos_labels[pos_ori_mask],
                    batch_idxs=pos_batch_idxs[pos_ori_mask],
                    pos_gt_ids=pos_gt_ids[pos_ori_mask]
                )

            if self.use_loss_ourwater:
                val_loss_ourwater = self.loss_ourwater(
                    pos_rbox_preds[pos_ori_mask],
                    pos_pseudo_targets.detach()[pos_ori_mask]
                )

        # [修改] 动态打包字典返回
        return dict(
            loss_cls=loss_cls,
            loss_bbox=val_loss_bbox,
            loss_bbox_vor=val_loss_vor,
            loss_bbox_ovl=val_loss_ovl,
            loss_bbox_edg=val_loss_edg,
            loss_ss=val_loss_ss,
            loss_size=val_loss_size,
            loss_angle=val_loss_angle,
            loss_ourwater=val_loss_ourwater
        )

    def get_targets(
        self, points: List[Tensor], batch_gt_instances: InstanceList,
        batch_pseudo_boxes: Optional[List[Tensor]] = None # [新增]
        ...
        labels_list, bbox_targets_list, bid_targets_list, pseudo_targets_list, gt_inds_list = multi_apply(
            ...
            batch_gt_instances,
            batch_pseudo_boxes,   # [核心修复]: 把它作为并行的位置参数！让框架逐一解包
        ...
        pseudo_targets_list = [
            pseudo_targets.split(num_points, 0) for pseudo_targets in pseudo_targets_list
        ]
        gt_inds_list = [
            gt_inds.split(num_points, 0) for gt_inds in gt_inds_list
        ]
        ...
        for i in range(num_levels):
            ...
            concat_lvl_bid_targets.append(torch.cat([bid_targets[i] for bid_targets in bid_targets_list]))
            concat_lvl_pseudo_targets.append(torch.cat([pseudo_targets[i] for pseudo_targets in pseudo_targets_list]))
            concat_lvl_gt_inds.append(torch.cat([gt_inds[i] for gt_inds in gt_inds_list]))
            
        return (concat_lvl_labels, concat_lvl_bbox_targets, concat_lvl_bid_targets, 
                concat_lvl_pseudo_targets, concat_lvl_gt_inds)

    def _get_targets_single(
            self, 
            gt_instances: InstanceData, 
            pseudo_boxes: Optional[Tensor], # [核心修复]: 移到这里接收解包后的单图 Tensor
            points: Tensor,
            regress_ranges: Tensor,
            num_points_per_lvl: List[int]) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        if num_gts == 0:
            ...
            return gt_labels.new_full((num_points,), self.num_classes), \
                   ...
                   gt_labels.new_full((num_points,), -1, dtype=torch.long)
        ...
        if pseudo_boxes is not None and len(pseudo_boxes) > 0:
            valid_inds = min_area_inds[valid_pos_mask].clamp(0, len(pseudo_boxes) - 1)
            point_pseudo_targets[valid_pos_mask] = pseudo_boxes[valid_inds]
