...
model = dict(
    ...
    bbox_head=dict(
        ...
        edge_loss_cls=[0, 2, 3, 4, 6, 7, 8, 10, 11],
        angle_coder=dict(
            type='PSCCoder',
            angle_version='le90',
            dual_freq=False,
        ...
        loss_angle=dict(
            type='AngleLoss',
            loss_weight=1.0,     # [保留原有设定] 损失权重为 2.0
            k_radius=2.0,        # [保留原有设定] 高斯邻域半径系数为 2.0
            score_alpha=1.0,     # 置信度权重幂次
            target_classes=[0,1,2,6,7,8,9,10,11], # 指定计算类别。None 表示全类别开启。
        ...
        loss_size=dict(
            type='SizeLoss', 
            loss_weight=1.0,     # [保留原有设定] 损失权重为 0.6
            beta=1.0,
            target_classes=[0,1,2,6,7,8,9],
            topk=0.95
        ),
        ...
        loss_ourwater=dict(
            type='OurWaterLoss',
            loss_weight=1.0      # 参考之前的设置，可根据实际效果微调
        )),
...
train_pipeline = [
    ...
    # === [关键插入] 加载离线生成的伪标签 ===
    # ⚠️ 请确保这里的路径替换为你 CODrone 真实生成的 pkl 路径 ⚠️
    dict(type='LoadPseudoAnnotations', 
         pkl_path='/mnt/data/xiekaikai/split_ss_codrone/trainval/trainval.pkl'),
    # ========================================
...
optim_wrapper = dict(
    optimizer=dict(
        ...
        betas=(0.9, 0.999),
        weight_decay=0.05),
    clip_grad=dict(max_norm=35, norm_type=2))  # [保留原有设定] 梯度裁剪
