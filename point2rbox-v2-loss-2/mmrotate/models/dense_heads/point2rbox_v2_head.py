...
#         # [新增] 构建透视感知尺寸一致性损失
#         self.loss_pascl = MODELS.build(loss_pascl)
...
#         # [Step B] Construct Batch Index for flattened tensors
#         batch_idxs_list = []
#         for points in all_level_points:
#             num_points_lvl = points.size(0)
#             # Create batch index: [0]*N, [1]*N, ... for N points
#             lvl_batch_idxs = torch.arange(num_imgs, device=points.device).view(-1, 1).expand(-1, num_points_lvl).reshape(-1)
#             batch_idxs_list.append(lvl_batch_idxs)
#         flatten_batch_idxs = torch.cat(batch_idxs_list)
...
#         loss_bbox_edg = None
#         loss_ss = None
#         loss_naoa_value = None
#         # [新增] 初始化新 Loss 变量
#         loss_pascl_value = None

#         # 获取各个 Loss 的权重开关
#         calc_cls = self.loss_cls.loss_weight > 0
#         calc_bbox = self.loss_bbox.loss_weight > 0
#         calc_ovl = self.loss_overlap.loss_weight > 0
#         calc_vor = self.loss_voronoi.loss_weight > 0
#         calc_edg = self.loss_bbox_edg.loss_weight > 0
#         calc_ss = self.loss_ss.loss_weight > 0
#         calc_naoa = self.loss_naoa.loss_weight > 0
#         # [新增] 获取新 Loss 开关
#         calc_pascl = self.loss_pascl.loss_weight > 0
#         # -----------------------------------------------------------
...
#         pos_batch_idxs = flatten_batch_idxs[pos_inds] # [Step B] Get positive batch indices
...
#             # -----------------------------------------------------------
#             # [关键] 必须更新 pos_rbox_targets，因为其他 Loss 依赖它
#             # -----------------------------------------------------------
...
#             # [修改] 仅当需要计算时才初始化 Tensor，避免不必要的显存分配
#             if calc_ovl: loss_bbox_ovl = ori_mu_all.new_tensor(0)
#             if calc_vor: loss_bbox_vor = ori_mu_all.new_tensor(0)
            
#             # [修改] 如果两个 Loss 都不计算，直接跳过循环
#             if calc_ovl or calc_vor:
...
#             # [Step C] Calculate NAOA Loss
#             # [修改] 增加开关判断
#             if calc_naoa:
#                 loss_naoa_value = self.loss_naoa(
#                     pos_rbox_preds,
#                     pos_cls_scores,
#                     pos_labels,
#                     pos_batch_idxs
...
#             # [新增] 计算透视感知尺寸一致性损失
#             if calc_pascl:
#                 loss_pascl_value = self.loss_pascl(
#                     pos_rbox_preds,
#                     pos_cls_scores,
#                     pos_labels
#                 )
...
#             # [修改] 只有计算了才进行除法
#             if calc_ovl: loss_bbox_ovl = loss_bbox_ovl / len(batch_gt_instances)
#             if calc_vor: loss_bbox_vor = loss_bbox_vor / len(batch_gt_instances)
...
#         else:
#             # [修改] 在没有正样本的分支中，也只计算开启的Loss
#             # 使用 sum() 是为了保持计算图连接（欺骗DDP检查），防止 "unused parameter" 报错
#             if calc_bbox: loss_bbox = pos_bbox_preds.sum()
#             if calc_vor: loss_bbox_vor = pos_bbox_preds.sum()
#             if calc_ovl: loss_bbox_ovl = pos_bbox_preds.sum()
#             if calc_edg: loss_bbox_edg = pos_bbox_preds.sum()
#             if calc_ss: loss_ss = pos_bbox_preds.sum()
#             if calc_naoa: loss_naoa_value = pos_bbox_preds.sum()
#             # [新增] 新 Loss 的空值处理
#             if calc_pascl: loss_pascl_value = pos_bbox_preds.sum()

#         # [修改] 动态构建返回字典，自动过滤掉为 None (未计算) 的 Loss
#         losses = dict()
#         if loss_cls is not None:
#             losses['loss_cls'] = loss_cls
#         if loss_bbox is not None:
#             losses['loss_bbox'] = loss_bbox
#         if loss_bbox_vor is not None:
#             losses['loss_bbox_vor'] = loss_bbox_vor
#         if loss_bbox_ovl is not None:
#             losses['loss_bbox_ovl'] = loss_bbox_ovl
#         if loss_bbox_edg is not None:
#             losses['loss_bbox_edg'] = loss_bbox_edg
#         if loss_ss is not None:
#             losses['loss_ss'] = loss_ss
#         if loss_naoa_value is not None:
#             losses['loss_naoa'] = loss_naoa_value
#         # [新增] 添加新 Loss 到返回字典
#         if loss_pascl_value is not None:
#             losses['loss_pascl'] = loss_pascl_value
...
class Point2RBoxV2Head(AnchorFreeHead):
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            # pos_rbox_preds: [cx, cy, w, h, a]
            # pos_cls_scores: scores
            # pos_labels: labels
            
            # Fix: Extract batch indices from pos_bid_targets (column 0 is batch_id)
            ...
            loss_perspective = self.loss_perspective(
                ...
                labels=pos_labels,
                batch_idxs=pos_batch_idxs,  # Added this
                img_metas=batch_img_metas   # Added this
            )
